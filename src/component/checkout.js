...
var Checkout = () => {
	...
	var fetchcart = async () => {
		try {
			const resp = await fetch(`http://localhost:9000/api/fetchcart/${username}`)
			if (resp.ok) {
	...
	var saveorder = async () => {
		...
		var orderdata = { address, orderamt: sessionStorage.getItem("billamt"), pmode, uname: username, carddetails, status: "Payment Received, processing", cartdata }
		...
		try {
			...
			if (resp.ok) {
				...
				if (result.statuscode === 1) {
					...
					toast.error("Error while update stock");
				}
				else if (result.statuscode === -2) {
					toast.error("Error while empty cart");
				}
			}
			else {
				toast.error("Error Occured")
			...
			<div className="login">
				<div className="container">
					...
					<div className="login-form-grids animated wow slideInUp" data-wow-delay=".5s">
						...
						<input type="button" className="btn btn-primary" onClick={oncheckout} value="confrim order" /><br /><br />
					</div>
